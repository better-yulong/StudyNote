...
2. 第2个合理用途与对象的本地对等体(native peer)有关。本地对等体是一个本地对象(native object),普通对象通过本地方法(native method)委托给一个本地对象。因为本地对等体并不是一个普通对象，所以垃圾回收器不知道它，当它的Java对等体被回收时，其并不会回收。如果本地对等
