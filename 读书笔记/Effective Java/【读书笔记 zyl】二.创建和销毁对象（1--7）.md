...
- 终结方法的缺点是不能保证会被及时地执行（JVM异步线程执行且不保证一定可执行成功，即可能很长时间才会执行）。如使用终结方法关闭已打开的文件、流，是严重的错误思路。
- 及时执行终结方法是垃圾回收算法的一个主要功能，但算法实现在不同的JVM实现会大相径庭，同样的程序运行在不同的JVM表现可能会截然不同。不正常的使用终结方法，可能会随意的延迟其实例的回收过程，因JVM异步执行终结方法线程的优先级比应用程序其他线程的低很多。