...
- 显示终止方法最典型的例子是InputStream、OutputStream和java.sql.Connection的close方法；另外一个则是java.util.Timer的cancel方法，它改变状态，使得与Timer实例相关联的的线程温和的终止自己。显示的终止方法通常与try-finally结构结合使用，以确保及时终止。在finally子句内部显