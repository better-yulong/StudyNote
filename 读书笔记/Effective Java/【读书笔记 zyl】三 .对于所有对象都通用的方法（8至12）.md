...
- 基于如上理角提到的逻辑相等，个人也想到==是否可认为物理相等。那==对于引用类型的比较一定是只有指向同一个对象才会返回true；而Object类的equals方法默认其实也是使用==判断，但是因为不同的类、场景需要，很多类都对equals&hashCode方法进行覆盖重写，使得其支持逻辑相等的判断。下面来看下String类的equals及hashCode方法源码
```language
