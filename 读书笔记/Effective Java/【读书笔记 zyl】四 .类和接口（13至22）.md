...
- 在仔细设计你的类的公共API后，最先的反应应该是让所有其他成员设计为私有，只有当同一个包中的其他类真的需要访问成员时，需要删除私有修饰符将其调整为包级私有。私有成员和包级私有成员应该是类实现的一部分，通常不会影响其导出的API。但是，如果类实现Serializable接口，则这些属性可以泄漏到导出的API中。（后续章节）
