...
```
- 此处无疑指出了一个习惯，即所有类最好都显示构造器，且不可实例化的类需针对性防范被实例化。AssertionError不是必须的，但是它可避免不小心在类的内部调用构造器，可保证该类在任何情况下不会被实例化。这种习惯会让人感觉不友好，明智的做法则在代码中增加明确的注释。但此种方式也有副作用，即该类不能被子类化，因子类构造器都必须显示或隐式的调用超类(superclass)构造器，此种情况即会导致可访问的