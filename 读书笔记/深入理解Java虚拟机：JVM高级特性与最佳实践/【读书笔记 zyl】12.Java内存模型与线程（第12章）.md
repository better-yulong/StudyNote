...
原子性（Atomicity）：由Java内存模型直接保证原子性变量操作包括read、load、use、assign、store和write，可认为所所有基本数据类型的访问读写均具备原子性（long、double虽然非原子协定例外，但几乎不会发生例外）。如若应用场景需更大范围的保证原子性，Java内存模型则提供
