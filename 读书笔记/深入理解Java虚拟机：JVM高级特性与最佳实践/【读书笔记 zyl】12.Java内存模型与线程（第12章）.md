...
可见性(Visibility）:可见性是指一个线程修改共享变量的值，其他线程能够立即得知这个修改。Java内存模型是通过变量修改后将新值同步回主内存，而变量读取前从主内存刷新新值至工作内存的方式不实现可见性。普通变量与volatile变量都是如此，但区别在于volatile的特殊规则保证修改为新值后能立即同步至主内存，以及每次使用前立即从主内存刷新。因此，volatile保证多线程操作时
