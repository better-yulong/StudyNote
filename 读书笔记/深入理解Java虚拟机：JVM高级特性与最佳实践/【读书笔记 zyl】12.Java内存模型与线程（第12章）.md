...
而狭义上用户线程指的是完全建议在用户空间的线程库上，系统内核不能感知线程存在的实现。用户线程的建立、同步、销毁和调度完全在用户态完成，无需内核的帮助。若程序实现得当，用户线程无需切换到内核态，因此操作是快速且低消耗，也可支持规模更大的的线程数量，部分高性能数据库的多线程就是由用户线程实现。这种进程与用户线程之间1：N的
