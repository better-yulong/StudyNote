...
LockSupport的park/unpark和Object的wait/notify:面向的对象不同；跟Object的wait/notify不同LockSupport的park/unpark不需要获取对象的监视器；实现的机制不同，因此两者没有交集。虽然两者用法不同，但是有一点， LockSupport 的park和Object的wait一样也能响应中断.（参考：https://blog.csdn.net/fenglllle/article/details/83049251）
