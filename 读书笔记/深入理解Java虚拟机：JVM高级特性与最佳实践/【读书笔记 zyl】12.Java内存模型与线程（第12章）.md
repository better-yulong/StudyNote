...
> 如把一个变量从主内存复制到工作内存，则顺序执行read、load操作，如把一个变量从工作内存同步到主内存，则顺序执行store、write操作。Java内存模型要求上述两个操作必须顺序执行，但没有保证连续执行。即read与load之间、store与write之间可插入其他指令，如对主内存的变量a、b进行访问，一种可能出现的顺序是read a、read b、load b、load a。除此之外，Java内存模型还规定执行上述8种基本操作时必须满足如下