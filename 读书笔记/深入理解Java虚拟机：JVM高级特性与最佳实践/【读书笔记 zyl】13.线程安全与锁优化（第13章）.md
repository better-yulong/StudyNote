...
2. sleep(long)、suspend、resume、yield、join、interrupt、interrupted、isInterrupted这些方法均为Thread类的方法，并不要求必须运行于同步方法（代码块）
   ...
   - interrupt 中断线程。若线程在调用 Object 类的 wait()、wait(long) 或 wait(long, int) 方法，或者该类的 join()、join(long)、join(long, int)、sleep(long) 或 sleep(long, int) 方法过程中受阻，则其中断状态将被清除，它还将收到一个 InterruptedException。若线程t1处于sleep状态，那么在线程t2中运行 t1.interrup
