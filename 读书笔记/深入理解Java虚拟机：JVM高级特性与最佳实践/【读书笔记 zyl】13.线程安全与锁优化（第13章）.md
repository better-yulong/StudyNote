...
- 基于轻量级锁来理解：假设当前虚拟机启动用偏向锁（启用参数-XX:+UseBiasedLocking,其为JDK1.6的默认值），那么当锁对象第一次被线程获取时，虚拟机会把对象头标志位设为"01"，即偏向模式。同时使用CAS操作把获取到这个锁的线程ID记录在对象的Mark Word中；如果CAS操作成功，持有偏向锁的线程以后