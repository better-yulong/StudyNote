...
若更新操作失败，虚拟机首先会检查对象的Mark Word是否指向当前线程的栈帧，如果是则说明当前线程已经拥有这个对象的锁那就可直接进入同步块继续执行，否则说明该对象已经被其他线程抢占。如果有两条以上的线程争用同一个锁，那轻量级锁就不再有效，要膨胀为重量级锁，锁标志的状态值变为"10"，Mark Word中存储的就是指向重量级锁（互斥量）的指针，