...
2. 判断 threadLocals 是否为空；首次为空则调用 createMap(t, value)，该方法逻辑也简单，以当前ThreadLocal对象为key输入的参数为value，创建ThreadLocalMap（基于ThreadLocal对象、value创建ThreadLocalMap，而ThreadLocalMap底层实际是Entry数组）----该分支只会执行一次
3. 判断 threadLocals 不为空，即已完成ThreadLocalMap初始化，此时执行map.set(this, value)。而ThreadLocalMap底层是实际是一个Entry数组（Entry则是基于ThreadLocal对象及其value的封装）。


set方法先是遍历数组，若数组已存在该ThreadLocal对象（即之前已设置值，此处非首次设置则是更新）则直接用新的value替换原值即可。