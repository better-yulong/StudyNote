...
- 那轻量级锁的解锁过程呢？它的解锁过程也是通过CAS操作来进行，如果对象的Mark Word仍指向线程的锁记录，那就用CASe操作把对象当前的Mark Word和线程中复制的Displaced Mark Word 替换回来；如果替换成功则整个同步完成。如果替换失败则说明有其他线程尝试过获取该锁，那就要在释放锁的同时唤醒被挂起的线程。