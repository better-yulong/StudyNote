...
###### 1. 不可变
Java语言中（特指JDK1.5之后，即Java内存模型被修正之后的Java语言），不可变(Immutable)的对象一定是线程安全的，无论是对象的方法实现还是方法的调用者，都不需要采取任何的线程安全保障措施。如final关键字提到的可见性，只要一个不可变的对象被正确的构造出来（即未发生this引用逃逸的情况），