...
- 简单的说，如 ThreadLocal<String> t4 = new ThreadLocal<String>;String t2s = new String("t2");t2.set(t2s);这3行代码，基于set方法及Entry的类继承（Entry extends WeakReference<ThreadLocal>），ThrealLocal及value同时会作为Entry的两个属性保存在Entry对象。如若按平时，更多是将ThreadLocal对象t4