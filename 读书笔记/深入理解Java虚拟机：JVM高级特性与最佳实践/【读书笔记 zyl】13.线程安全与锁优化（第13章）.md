...
```
ThreadLocalMap是ThreadLocal类的一个static静态内部类(static不能修饰顶级类（top level class），只有内部类可以为static；静态内部类仍然）。ThreadLocal的set为首先取当前线程Thread对象t，根据对象t判断是否已有对应的ThreadLocalMap对象