...
- 该定义相对严谨，它要求线程安全的代码都必须具备一个特征：代码本身封装了所有必要的正确性保障手段（如互斥同步等），调用者无须关心多线程的问题，更无须自己采取任何任何措施是来保证多线程的正确调用。但实际很难做到，故大多数场景会把定义弱化：把"调用这个对象的行为"限定为"单次调用"，若满足该条件也可称为线程安全（见后面的
