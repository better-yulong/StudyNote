...
实际，大多数方法是无法内联，其原因是第8章讲解Java方法解析和分派调用，只有使用invodespecial指令调用的私有方法、实例构造器、父类方法以及使用invokestatic指令进行调用的静态方法才可在编译期解析，其他方法因涉及到多态选择且可能存在多于一个版本的方法接收者（final方法修饰符
