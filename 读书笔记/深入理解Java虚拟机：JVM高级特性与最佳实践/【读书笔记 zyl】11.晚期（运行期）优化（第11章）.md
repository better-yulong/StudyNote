...
编译器内联，如果为非虚方法则直接进行内联，此时的内联是稳定前提保障的。若是虚方法，则会向CHA查询此方法在当前程序是否有多个目标版本可供选择，如果查询只有一版本则也可进行内联，不过此种内联属于激进优化，需要预留一个逃生门，称为守护内联（Guarded Inlining)。若程序后续执行过程中，虚拟机未加载到会令这个方法的接收者
