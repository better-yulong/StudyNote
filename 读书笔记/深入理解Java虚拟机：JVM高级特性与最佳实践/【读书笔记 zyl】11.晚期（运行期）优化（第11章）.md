...
##### 2. 公共子表达式消除
公共表达式消除是普遍应用于各种编译器的经典优化技术，其含义是：如果一个表达式E已计算过，且从先前计算到现在E中所有变量的值都没有发生变化，那么E的这次出现就成为了公共表达式；对于这种表达式，直接用先前计算过的表达式结果代替E即可。若公共子表达式消除优化仅限于程序的基本块内，称为局部公共子表达式消除；若这种优化涵盖多个基本块，则称为全局公共子表达式
