...
Code属性详细说明是每个method的Code属性，而这个Code属性仅是包含源码方法{}内的代码，而在{}内部使用的泛型相关代码则只能使用Object替换（声明在接口、类、方法上）或者强制类型转换（如方法内容声明List<Integer>并调用get方法时）根据之前章节可知道Code属性可认为即是方法内部代码编译后的JAVA操作指令。
