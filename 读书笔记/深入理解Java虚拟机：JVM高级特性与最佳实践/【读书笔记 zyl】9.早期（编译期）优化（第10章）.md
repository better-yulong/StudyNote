...
```
- 只有使用条件为常量的if语句才可达到该效果（true、false或者类似1==1这类全部基于常量可直接基于常量折叠为true、false的表达式才可以，其他复杂如表达式 Integer.valueOf(1)==Integer.valueOf(1) 都不可以）,经验证JDK7时while(1==1)也会直接基于条件编译
