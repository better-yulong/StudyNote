...
4. 那么更进一步，涉及到类与类型？注意区分类型（Type）与类（Class）的区别，这里Class是Type的一种是Type的子集，直接子类只有一个也就是Class，代表着类型中的原始类型以及基本类型，而像数组、枚举等“类型”是相对于Class来说。如method.getGenericReturnType() 可以理解获取源码定义的类型信息信息（如泛型A），
