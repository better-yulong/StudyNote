...
```
- 对于public static Integer method1_1(ArrayList<Integer> list)、public static void method1_1(ArrayList<String> list) 两个方法，在JDK1.6 可重载成功，对以前重载仅以方法名和参数来判断而返回值不参与重载选择的认知确实是个挑战。其实JDK1.6 也不是根据返回值来确定重载，只是因为方法返回