...
- 如之前章节提到的实例构造器方法<init>()和类构造器方法<clinit>()即是在该阶段添加到语法树（实例构造器并不是指默认构造函数，若代码中没有提供任何构造函数，那编译喊叫则会添加一个无参、访问修饰符（public、protected或private）与当前类一致的默认构造函数，该工作在填充符号表阶段就已经完成）。这两个构造