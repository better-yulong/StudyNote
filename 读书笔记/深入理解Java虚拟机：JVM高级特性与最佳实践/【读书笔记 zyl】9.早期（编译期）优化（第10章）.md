...
```
- 通过如上GenericTypesTest2类源码及反编译、二进制示例相对理解比较清晰，泛型即为类型参数化。即然是参数，那么就涉及到参数的声明、作用域、类型信息。而泛型就可以理解为可以动态指定实际类型的参数。对于很多地方理解的如泛型接口、泛型类、泛型方法的分类其实不太苟同，个人感觉只不过是泛型参数声明位置、使用域有区别布局，并非排它而是可组合。
