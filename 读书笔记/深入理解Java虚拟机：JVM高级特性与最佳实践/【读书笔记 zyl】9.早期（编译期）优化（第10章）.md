...
那元数据又怎么理解呢？其实可以得意理解为Class的骨架信息，或者对我们而言就是可以通过反射机制操作的数据（Constructor类、Field类、Method类）。那接口、类字义的泛型信息在Attributes数组的Signature属性会保留，而方法的泛型信息则在method的Signature属性保留）。也就可以理解
