...
```
2. 泛型作用域：因泛型不同于已知类型，而变量通常包含类型、参数名。故规范确认为<A>声明泛型A,而其作用域与实例变量、局部变量、方法入参相同。即定义在class、interface 关键字后的泛型可使用于实例变量、方法入参、方法内、方法返回；而定义在方法的泛型使用域则为 。而这就一个点，因泛型是动态类型，故泛型参数均不能使用static修饰，但可使用final修饰。
