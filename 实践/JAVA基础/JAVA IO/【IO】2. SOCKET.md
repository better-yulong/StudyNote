...
```
其原因呢？时OutputStream为abstract 抽象类，而通过debug发现SocketClient类的socket.get java.net.SocketOutputStream， flush()默认为空方法，即调用没有任何作用；而只有SocketClient的out.close()方法：会自动刷新缓冲区然后关闭流对象。
```language
