...
3. 异常场景：
   ...
   3. 如若SocketClient在write循环执行（还未执行到close()方法）， SocketServer突然关闭SocketClient会报错：Connection reset by peer: socket write error；但若相反是SocketClient突然关闭
   3.  若SocketServer突然关闭，此时SocketClient抛出异常：
