...
原因其实就在每次文件读取和写入。简单说下最原始的方法分几步：1. 生成二维码图片；2.基于二维码图片添加logo ；3.基于前一步图片添加title文件；4. 基于前一步图片添加 tail文件。整个过程中每步都读取文件、且在磁盘写入文件， 此种方法好处在于理解简单，而且在前期调试非常方便，可以确认每步处理结果。但从上面分析就可以看出IO导致的性能变差
