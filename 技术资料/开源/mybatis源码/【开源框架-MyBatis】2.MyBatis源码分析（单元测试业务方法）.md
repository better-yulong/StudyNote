...
- 基于JDK Proxy动态代理，可知当调用Execute的实例时会被代理调用Plugin的invoke方法，而从invoke方法源码可发现如若当前方法上面SignatureMap中则通过调用FlushCacheInterceptor为的intercep方法（对原方法拦截，在执行前添加了缓存判断、flush相关逻辑，之后再执行调用原始的方法）；如若调
