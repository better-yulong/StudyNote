...
- MapperAnnotationBuilder的parese方法则是判断是否加载过（namespace，即对应Dao的类名：interface domain.blog.mappers.BlogMapper）。先行标记，然后运行loadXmlResource（可忽略）；之后是调用parseCache()、parseCacheRef()则是判断当前Dao方法
