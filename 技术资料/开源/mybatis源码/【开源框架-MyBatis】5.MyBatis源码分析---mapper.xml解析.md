...
```
javaTypeClass值：javaType不为空则直接返回。若为空,jdbcType为CURSOR时其为ResultSet.class，否则从parameterMap中的type对应的类（resultType传入的为parameterMap中的type对应的类）从判断其对应的get方法的返回值。如若最后仍无法匹配到，则初始化
```language
