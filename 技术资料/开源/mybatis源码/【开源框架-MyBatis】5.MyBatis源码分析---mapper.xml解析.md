...
从源码看，parameterMap仅有两个属性：id、type.其中id为字符串，而type则需可解析成class类（如若type无法）。细看Class parameterClass = resolveClass(type) 会发现type接受的值为两种：1.别名alais，此时会从MapperConfig.xml的typeAliases显示配置及自动初始化的基本类型、数组及集合类在TypeAliasRegistry去匹配；2.class完整名称，