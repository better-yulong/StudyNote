...
```
setupFields()方法根据
commandName根据Proxy0实例的接口名称（AuthorMapper）及当前执行的方法selectAllAuthor拼接设置commandName，setupCommandType则是根据commandName调用config.getMappedStatement(commandName)方法获取MappedStatement对象及CommandType，validateStatement方法为验证MappedStatement
