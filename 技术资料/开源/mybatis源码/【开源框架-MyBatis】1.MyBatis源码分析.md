...
- 但从上面的drop报错也引身另外一个问题，即是重复执行时必须先执行drop说明上次执行初始化的表及数据仍存在于内存。这块儿有些不太理解，原本认为每次Junit执行均会创建新的JVM进程，故每次理应都是新环境，但实际不是？可参考笔记"【JUnit】JUnit原理分析",实际JUnite对应的业务方法在eclipse 自身的JVM中执行，故首次运行时即已缓存至JVM，后续执行需先行drop表重新初始化数据。