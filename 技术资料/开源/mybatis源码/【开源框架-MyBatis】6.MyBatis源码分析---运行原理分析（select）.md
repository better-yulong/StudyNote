...
- getRowValue方法（getRowValue(rs, discriminatedResultMap, null)如下核心方法）
  1.createResultObject()方法用于创建结果对象，但参数中的ResultMap是上一步获取的discriminator对应的子ResultMap，即创建的是子ResultMap对应的实例对象（其中会判断ResultMap的type对应的class；如若class在TypeHandler中则直接基于该ResultMap创建对应对象（如Integer、Date，抑或自定义对象）；如若class不在TypeHandler中
