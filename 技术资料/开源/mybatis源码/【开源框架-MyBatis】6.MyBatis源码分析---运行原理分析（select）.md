...
- getRowValue方法（getRowValue(rs, discriminatedResultMap, null)如下核心方法）
  1.createResultObject()方法用于创建结果对象，但参数中的ResultMap是上一步获取的discriminator对应的子ResultMap，即创建的是子ResultMap对应的实例对象（其中会判断ResultMap的type对应的class；如若class在TypeHandler中则根据ResultMap的Type创建对应对象（如Integer、Date，抑或自定义对象）；如若有constructor标签，则调用获取rs对应的值调用有参构造方法实例化对象；如若classg不在TypeHandler也无constructor标签|column1|column2|column3|
|-|-|-|
|content1|content2|content3|
则直接调用objectFactory.create(resultType)创建对象，就对应于resultType="domain.blog.Author"）
