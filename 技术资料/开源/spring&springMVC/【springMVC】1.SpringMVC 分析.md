...
- 完成BeanNameUrlHandlerMapping、DefaultAnnotationHandlerMapping基于beanName或者RequestMapping解析生成hanlder之后，会将BeanNameUrlHandlerMapping、DefaultAnnotationHandlerMapping对应的两个实例对象赋值给DispatcherServlet实例，而后续http请求则会分别调用这两个实例进行url匹配与分析（先BeanNameUrlHandlerMapping匹配，后匹配DefaultAnnotationHandlerMapping），即同一URL会优先根据BeanName去匹配，如若未匹配到则再基于注解匹配
...
```
- 从日志来看，即未找到RequestHandlingMethod。通过调试发现，基于BeanNameUrlHandlerMapping会先根据url前面部分（/exampleController）获取Hanlder（实现与上面分析相似），但在获取到Hanlder之后（），则会再次获取HandlerAdapter实例ha，最终调用 ha.handle(processedRequest, response, mappedHandler.getHandler())完成实际调用并返回ModelAndView实例。补充一点哈，调用 ha.handle之前也会根据调用Interceptor）
...
未对404做特殊处理，可直接通过浏览器的404页面信息予以区分
