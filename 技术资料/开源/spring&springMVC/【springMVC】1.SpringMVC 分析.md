...
- 基于http://localhost:8080/springmvc3-analysis/example/helloWorld 访问时始终报404，经调试分析在匹配url之前会做几点处理：判断fullPath是否包含应用名（有可能是/）并处理；使用url匹配servlet配置（即对应wb.xml的servlet配置（<url-pattern>/example/*</url-pattern>）并生成servletName（会从url中把web.xml中可匹配的前缀去除但保留前面的/
