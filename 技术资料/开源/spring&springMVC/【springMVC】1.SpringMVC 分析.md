...
- 基于http://localhost:8080/springmvc3-analysis/example/helloWorld 访问时始终报404，经调试分析在匹配url之前会做几点处理：判断fullPath是否包含应用名（有可能是/）并处理；判断url是否可匹配到servletName（即对应wb.xml的servlet配置（
