...
- 既然有了上面的理解，那就知道上面的Linstener是无法实例在使用时实例化，而是基于注解注入（其实就是传说的DI：依赖注入）时就完成其实例化，那有没有方法基于上面的代码实现在调用lazyInitBeanExample.hashCode()才实例化呢？哈哈，其实后面的笔记有讲到Aware接口，发看就知道解决方案了（此处是猜测，马上开启验证）
