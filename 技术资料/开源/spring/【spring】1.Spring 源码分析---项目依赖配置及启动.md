...
- 针对XmlWebApplicationContext的loadBeanDefinitions方法做下细化说明：
  1. getConfigLocations 用于获取spring上下文匹配文件：该方法可用于获取默认/WEB-INF/applicationContext.xml； 或者根据getNamespace()获取类似spring-servlet.xml配置文件路径（此处也正好可协助用一起理解springmvc子上下文与spring父上下文；同时也明白如若要自定义实现默认的applicationContext.xml命名为application.xml则需考虑自定义实现getConfigLocations方法，）
