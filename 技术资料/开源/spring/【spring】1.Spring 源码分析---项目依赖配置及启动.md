...
Environment主要是负责解析properties和profile，最终通过过PropertySourcesPropertyResolver这个类来处理；如Environment的<T> T getProperty(String key, Class<T> targetType)为泛型参数，properties初始值得到的是String,需要ConversionService以及其相关类从String转换成T类型
