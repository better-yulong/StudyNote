...
```language
  //ReferenceConfig
	...
	initialized = true;
        ...
        appendProperties(this);////默认通过反射获取ReferenceConfig及其父类AbstractReferenceConfig属性值（基于System.getProterty方式），如check（检查服务提供者是否存在 ）、lazy（延迟创建连接）、reconnect（重连）、stubevent、version、group等
