...
```language
  //ReferenceConfig
	...
	initialized = true;
    	...
    	// 获取消费者全局配置(当前示例未配置)
    	checkDefault();//实例化默认ConsumerConfig，并通过反射获取ConsumerConfig及其父类AbstractReferenceConfig属性配置（基于System.getProterty方式），如check（检查服务提供者是否存在 ）、lazy（延迟创建连接）、reconnect（重连）、stubevent、version、group等
        appendProperties(this);////默认通过反射获取ReferenceConfig及其父类AbstractReferenceConfig属性配置（基于System.getProterty方式），如check（检查服务提供者是否存在 ）、lazy（延迟创建连接）、reconnect（重连）、stubevent、version、group等
