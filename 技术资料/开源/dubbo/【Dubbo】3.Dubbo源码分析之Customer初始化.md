...
```language
    ...
    public MockClusterInvoker(Directory<T> directory, Invoker<T> invoker) {
     ...
     //invoke方法：1.若是非Mock场景则是调用FailoverClusterInvoker的invoker方法（因为外层的invoker实际是MockClusterInvoker对FailoverClusterInvoker的包装；2.若是Mock场景则直接调用MockClusterInvoker自身的
