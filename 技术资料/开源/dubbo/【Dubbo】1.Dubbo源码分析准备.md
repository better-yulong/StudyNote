...
2. 再次切加dubbo /目录运行mvn clean install -Dmaven.test.skip，但又发现缺少hessian-lite依赖，于是首各种资料（尝试找源码编译、找可用的maven仓库），花了不少时间根据资料（https://github.com/apache/dubbo/issues/21）在mavne的 settings.xml文件添加mirror源：
```language
<mirror>  
       <id>cuisongliu</id>  
        <mirrorOf>*</mirrorOf>    
