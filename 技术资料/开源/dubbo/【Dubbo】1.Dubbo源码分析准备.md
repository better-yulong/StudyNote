...
1. 全局application配置（即对应ServiceBean类afterPropertiesSet()方法的处理全局application配置；此处注意还有个前提：即provider没有指定application，若已指定则忽略全局application），用于注册中心计算应用间依赖关系（可考试
```language
  <dubbo:application name="rpc-server"></dubbo:application>
```
...
###### 3.3 对应module配置(对应dubbo:module或者module--ModuleConfig)，也有三种方式同上；module配置主要用于模块信息配置，可用于
