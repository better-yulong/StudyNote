...
- 既然此处将xml解析生成各种BeanDefinition，那么根据之前分析spring过程，对应的Bean实例化之后可通过类似initial方式或者Processor方式来处理，于是乎调试看看。发现调试时始终无法关联到当前eclipse中的spring源码，而是关联到spring jar中的class反编译文件；定位发现dubbo
