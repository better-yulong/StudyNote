...
###### 3.3 对应module配置(对应dubbo:module或者module--ModuleConfig)，方式同上（可不配置、全局配置、Provider配置）；module配置主要用于模块信息配置，用于注册中心计算模块间依赖关系
###### 3.4 对应registry配置(对应dubbo:registry或者registry--RegistryConfig)，方式同上（可不配置、全局配置、Provider配置、Application配置）；module配置主要用于模块信息配置，用于注册中心计算模块间依赖关系
