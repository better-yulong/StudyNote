...
```
但验证时，发现调用dubbo服务始终报错：com.alibaba.dubbo.rpc.RpcException: No provider available in [invoker :interface com.aoe.demo.rpc.dubbo.DubboExampleInterf1 ->...；一开始纠结了比较久，但通过调试发现，dubbo:reference配置的mock配置仍会调用远程服务；至此，仍是花了比较久的时间，于是乎重新梳理了下思路，
