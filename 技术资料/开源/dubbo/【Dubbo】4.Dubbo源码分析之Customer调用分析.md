...
```
但验证时，发现调用dubbo服务始终报错：com.alibaba.dubbo.rpc.RpcException: No provider available in [invoker :interface com.aoe.demo.rpc.dubbo.DubboExampleInterf1 ->...；一开始纠结了比较久，但通过调试发现，dubbo:reference配置的mock配置仍会调用远程服务；至此，仍是花了比较久的时间，于是乎重新梳理了下思路，可如此理解（http://dubbo.apache.org/zh-cn/docs/user/demos/local-mock.html）：
```language
	public Result invoke(Invocation invocation) throws RpcException {
        ...
        if (value.length() == 0 || value.equalsIgnoreCase("false")){
        	...
        	//force:direct mock 强制调用doMockInvoke（本地Mock），不走远程调用
				...
				if (e.isBiz()) {
					...
					if (logger.isWarnEnabled()) {
		        		logger.info("fail-mock: " + invocation.getMethodName() + " fail-mock enabled , url : " +  directory.getUrl(), e);
                                        ...
                                        /如若服务端处理失败则执行本地Mok（比如消费端配置的mock="retrun null")，调用doMockInvoke（本地Mock）
