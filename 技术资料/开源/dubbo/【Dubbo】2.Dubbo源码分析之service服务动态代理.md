...
Wrapper.getWrapper（Class clazz)方法对clazz初始判断后调用Wrapper.makeWrapper(Class clazz)生成包装类（内部使用封装了Javassisst的ClassGenerator来动态生成包装类class，为方便查看基于Javassist动态生成包装类，故在ClassGenerator
```language
