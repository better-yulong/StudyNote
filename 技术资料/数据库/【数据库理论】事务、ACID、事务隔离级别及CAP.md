...
##### 2.2 深入理解ACID、事务隔离及锁
ACID主要针对于事务本身及事务性操作之间的约束；而事务隔离则是事务性之间的操作与非事务性操作（select）。而读提交（Read Committed）与可重复读（Repeated Red）的区别就在于事务中select查询（不包含select * for update）处理的区别。比如可重复读（Repeated Red）隔离级别时，事务中的select读操作会对数据行添加共享锁避免并发的update操作
###### 2.2.1 共享锁和排他锁
