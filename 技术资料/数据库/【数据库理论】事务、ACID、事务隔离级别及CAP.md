...
处理更新丢失、脏读和不可重复读取。读取数据的事务将会禁止写事务，但允许读事务，写事务则禁止任何其他事务。可通过“共享读锁”和“排他写锁”实现。（MySql的默认隔离级别：写事务排他锁且读共享锁，即写时同一数据行不允许任何事务（包括读、写事务），读事务允许读事务但禁止写事务）---因事务中的读加了共享锁，使得该事务提交之前不再允许修改数据，即同一事务中多个select结果始终一致）
