...
点3：开始错写成continue，发现SocketClient3 第一次执行SocketServer3可完整获取到请求的输入，但SocketClient3 第二次执行时发现SocketServer3没有任何反应。原来是因为该处如果是continue会导致始终在for循环处无限等待；而实际如果需要获SocketClient3 第二次请求的数据则需要跳出for去执行while循环中的accept