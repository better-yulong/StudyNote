 ...
 * */
```
点1：SocketServer3的while(true)循环简单是否有新的客户端请求；而客户端SocketClient3 每次请求完数据就会close即可认为客户端关闭的连接；此处是忙等。SocketClient3 每执行一次SocketServer3的Socket client = serverSocket.accept();就会检测到，然后待
